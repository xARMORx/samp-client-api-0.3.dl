...
pub struct CPlayerPool {  
    pub m_nLocalPlayerId: ID,
    pub __align: ::std::os::raw::c_int,
    pub m_szName: CStdString,
    pub m_pLocalPlayer: *mut CLocalPlayer,
...
pub struct CRemotePlayer {
    ...
    pub pad_1: [::std::os::raw::c_char; 64usize],
    ...
    pub pad_3: [::std::os::raw::c_char; 12usize],
...
impl CLocalPlayer {
    ...
    pub fn name(&self) -> Option<&str> {
        player_pool().and_then(|players| players.m_szName.as_str().ok())
    }

    pub fn id(&self) -> Option<i32> {
        player_pool().map(|players| players.m_nLocalPlayerId as i32)
    }
}
...
pub fn local_player<'a>() -> Option<&'a mut CLocalPlayer> {
    player_pool()
        .filter(|pool| !pool.m_pLocalPlayer.is_null())
        .map(|pool| unsafe { &mut *pool.m_pLocalPlayer })
}
